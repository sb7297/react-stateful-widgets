...
export default function Programmers() {
  ...
  const [featuredProg, setFeaturedProg] = useState(null);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredProg ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeaturedProg(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
